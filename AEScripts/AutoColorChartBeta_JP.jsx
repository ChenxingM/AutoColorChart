	...
	});
{
	...
	pickedColorDisp.preferredSize.width = 111;
...
*/
	...
	function createButtonsFromJson(jsonData) {
		...
		for (var imageName in jsonData) {
			...
			singleColorRadio.onClick = function() { userSelection.extractColorMode = "選択中クループのみ"; }
				...
				for (var itemKey in jsonData[imageName][numberKey]) {
					if (!addedKeys[itemKey]) {
						...
						btn.onClick = function(key) { 
							return function() { 
								if(userSelection.extractColorMode === "選択中クループのみ") extractOneColor(key);
								else if(userSelection.extractColorMode === "色見本全部") extractAllColor(key);
